...
function expDescFromYMD(exp) {
  ...
  return dateFromYMD('j<\\s\\up>S</\\s\\up> M'+y,exp);
}
...
function onAdChange(mutations) {
  var nodesWereAdded = mutations.filter(function(mut) {
    return mut.addedNodes.length > 0;
  }).length > 0;
  if ($('.ad.strat-top').height() > parseFloat($('.ad.strat-top').css('minHeight'))) {
    $('.ad.strat-top').css('minHeight', $('.ad.strat-top').height()+'px')
  }
}
...
$(document).ready(function() {
  ...
  observer.observe && $('.ad.strat-top').length && observer.observe($('.ad.strat-top')[0], {subtree: true, childList: true});
});
