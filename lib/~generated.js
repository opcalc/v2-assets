...
window.onerror = function(errorMsg, url, lineNumber, colNum, errorObj) {
		var stackTrace = (errorObj !== undefined) ? errorObj.stack : 'N/A';
...
function jq_exclusive_switch(switchObj, time, switchType){
  ...
  if (['tablet', 'mobile'].includes(viewStyle) && target.attr('data-switch-toggle')) {
		target.toggle(time);
	} else {
		target.show(time);
	}
...
function init_funcs(cfg){
	loadSliders();
	$('.jq_start_hidden').hide();
  if (viewStyle == 'mobile' || viewStyle == 'tablet') $('#menu .jq_exclusive_target').hide();
...
function checkRequest(rqId){
  ...
  if (requests[rqId]['status'] == REQ_OK || requests[rqId]['status'] == REQ_ERROR)
    ...
    clearInterval(requests[rqId]['checker']);
  else if ((timePast >= 3000 && (cmd=='newTab')) ||
        (timePast >= 7000 && cmd=='getStockPrice') ||
        (timePast >= 20000 && (cmd=='getOptions' || cmd=='calculate'))){
    inputError("Ok, it seems to be taking a while "+requests[rqId]['desc']+".  If it doesn't come good, try refreshing.");
    clearInterval(requests[rqId]['checker']);
  }
}
...
function newTab(strat, rqIdDontRequest){
  ...
  $('#content_holder').append("<div class=\"jq_exclusive_target\" id=\"newTab_req"+rqId+"\" name=\"strat_tabs\"><p><span class=\"loading\">Loading</span></p></div>");

  return false;
}
function newTab_return(reqData, data){
  if (data['strat-info'] != undefined && data['strat-info']['text'] != undefined){
    ...
    var code = "<li class=\"stratTab dialog_tab\"name=\"strat_tabs:strat_"+data['tabId']+"\" id=\"strat_"+data['tabId']+"_button\">"+
        "<div><a href=\"#\" name=\"strat_tabs:strat_"+data['tabId']+"\" class=\"jq_exclusive_switch name\" onclick=\"return showTab(this)\">New "+data['strat-info']['text']+"</a> "+
	...
	$('#content_holder').append("<div class=\"jq_exclusive_target stratContent\" id=\"strat_"+data['tabId']+"\" name=\"strat_tabs\">"+data['code']+
    "<div class='results' id='t0_results'>\r\n"+
...
var c_value=escape(value) + ";path="+escape(opts.path)+";expires="+exdate.toUTCString()+";max-age="+(opts.expiry_days * 60*60*24);
document.cookie=c_name + "=" + c_value;
}
