...
function renderRiskGraph(graphData, t_no){
    ...
    for(var i in dataIdx) if (dataIdx.hasOwnProperty(i)){
      ...
      dataLine = [
        parseFloat(x),
        parseFloat(roundTo(getDisValByType(graphData, (parseFloat(x) - uPrice) * 100),2))
      ];
      timeCount = 0;
    ...
    vMin = Math.min(
      vMin,
      parseFloat(roundTo(getDisValByType(graphData, (parseFloat(graphPriceMin) - uPrice) * 100), 2))
    );
    ...
    seriesData[0] = { lineDashStyle: [2, 2] };

    // var lowHighPrices = (function() {
    //   var priceRangeKeys = Object.keys(graphData.data);
    //   return [priceRangeKeys[0], priceRangeKeys[priceRangeKeys.length - 1]];
    // })();
    // header.push('Vs stock purchase');
    // colors.push('#999999');
    // lineGraphData.push([parseFloat(uPrice)]);
...
function render_finder_results(_outcomes, meta) {
  ...
  outcomes = _outcomes || [];
  if (outcomes.length) {
    rEle.append('<h2>Top 5 options</h2>');
    outcomes.map(
      function (outcome, i) {
