...
function getV3priceData(stock, reqData) {
  $.getJSON(V3_API_PATH + '/price/' + stock, function(d) {
    ...
    var newOptions = Object.keys(options).reduce(
      (newOptions, opKey) => {
        var newKey = opKey.substr(0, 4) + '-' + opKey.substr(4, 2) + '-' + opKey.substr(6, 2);
        newOptions[newKey] = {
...
function finder_find() {
  ...
  var strategies = {
    'option-purchase': $('#finder-strat-option-purchase').attr('checked') === 'checked',
...
function doFinder_calculation_return(reqData, data) {
  ...
  var aEle = $('<a />', { href: '#tab=' + reqData.tabReturnData.tabId, class: 'button button--outline smallButton' });
  ...
  if (outcomes.length) {
    ...
    outcomes.map(
      function (outcome, i) {
        ...
        if (['long-put', 'long-call'].indexOf(strat.metadata.stratKey) >= 0) renderIn_single_option_ele(oEle, strat);
...
function finder_clear_form() {
  ...
  $('#finder_priceDirection option[value=eq]').text(symb + ' @');
  $('#finder_priceDirection option[value=lt]').text(symb + ' below');
  $('#finder_priceDirection option[value=gt]').text(symb + ' above');
