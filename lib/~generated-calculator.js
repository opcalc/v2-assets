...
function getPrice_return(reqData, returnData){
  ...
  if (returnData.status == REQ_ERROR){
    ...
    if (returnData.data_status == 4) _gaq.push(['_trackEvent', 'Symbol not found', reqData.stock]);
    // !!! will probably throw error in getOption_return when that ajax req comes back - cancel it

  }else if (returnData.status == REQ_OK){
    _gaq.push(['_trackEvent', 'Symbol search', reqData.stock]);
    underlying_symb[curTab] = reqData.stock;

    // !!! add bid/ask/last to subtle-links
    if ($('#'+reqData.eleId+'-price').val() === "")
      $('#'+reqData.eleId+'-price').val(returnData.price.last);
    $('#'+reqData.eleId+'-curPrice').val(returnData.price.last);
...
function getOptions_return(reqData, returnData){
  ...
  if (returnData.status == REQ_ERROR){
    ...
    if (reqData.showSelect){// * they have opened the selector and are seeing the loading symbol
      ...
      _gaq.push(['_trackEvent', 'Options not found', reqData.stock]);
    ...
    opCache[reqData.stock] = returnData.options;
  ...
  if (errors){
    ...
    if ($('#t'+tNo+'_graph-newTab').attr('checked')){
      ...
      if (typeof fromClose !== 'undefined'){
        _gaq.push(['_trackEvent', 'Tabs', 'Re-calc in new tab']);
      }
...
function wrapInParensIfReq(tabNum, returnedVal, minOrMax) {
...
function doCalculation_return(reqData, returnData){
  ...
  $('#t'+reqData.tab_num+'_graph-priceMin').val(
    wrapInParensIfReq(
      reqData.tab_num,
      roundTo(stripNonNumeric(returnData.results.graph_priceMin), 2),
      'min'
    )
  );
  $('#t'+reqData.tab_num+'_graph-priceMax').val(
    wrapInParensIfReq(
      reqData.tab_num,
      roundTo(stripNonNumeric(returnData.results.graph_priceMax), 2),
      'max'
    )
  );
  ...
  if (returnData.status == REQ_ERROR){
    ...
    _gaq.push(['_trackEvent', 'Calculation: '+returnData.results.strat, returnData.results.underlying_symb, returnData.tab_desc_long.desc.substring(returnData.results.underlying_symb.length+1)]);
...
function initOptionsSet(self, data){
    ...
    //_gaq.push(['_trackEvent', 'Option selected', data.stock, $('#'+data.eleId+'-act').val() + " " + opType, parseFloat(strike)]);
    ...
    $('#'+data.eleId+'-curPrice').val(price);
    $('#'+data.eleId+'-strike').val(strike);
    $('#'+data.eleId+'-expiry').val(exp);
    var y = dateFromYMD('Y',exp) == date('Y') ? '' : ' '+dateFromYMD('Y',exp); // only show year if not this year
    $('#'+data.eleId+'-opDesc').html(dateFromYMD('j<\\s\\up>S</\\s\\up> M'+y,exp)+' $'+strike+' '+(opType == 'c'?'Call':'Put'));
...
function renderRiskGraph(graphData, t_no){
  var lineGraphData = ['header_place_holder'];
    ...
    var timeCount, x, tHeader = [], vMax = -100000, vMin = 100000, tVal,
        colorPalette = [
    ...
    for(var i in dataIdx){
      ...
      for(var t in graphData.data[x]) if (graphData.data[x].hasOwnProperty(t)){
        ...
        if ((tTimeLen - t) % graphTimeScale == 0){
          ...
          if (false === header){
            tHeader.push(date('j M', (parseInt(graphData.start_date,10) + (3600*24) * t)));
            if (timeCount > 0){
              ...
              colors.push(colorPalette[(timeCount-1) % colorPalette.length]);
            }
            timeIndex.push(t);
          }
    ...
    header.push('Current');
    colors.push('#666666');
    lineGraphData.push([parseFloat(results[t_no]['underlying_current_price'])]);
    ...
    var options = {
      ...
      chartArea: {width: "75%", left:90, top:20, height:"85%"},
      vAxis:{title:(['riskGraphPrice', 'riskGraphRawValue'].includes(graphData.graph_type) ? 'Dollars' : 'Percent'),
        format: ['riskGraphPrice', 'riskGraphRawValue'].includes(graphData.graph_type) ? '$#,###.##' : '#.##\'%\'',
        viewWindowMode: 'maximized'   },
      hAxis:{title:"Underlying price", format: "$#,###.##"}
    };

    if (!['price', 'riskGraphPrice', 'riskGraphRawValue'].includes(graphData.graph_type)){
...
function renderGraph(graphData, t_no){
  ...
  for (var x in graphData.data) if (graphData.data.hasOwnProperty(x)){
    if (c==0){
...
function changeOutputType(selectEle){
  ...
  _gaq.push(['_trackEvent', 'Change Output type', type]);
...
function getSummary(results){
  ...
  var maxRiskString = (s.maxrisk.g == 'u' ? 'infinite on upside' :
    '<span'+(mP ? ' style="font-weight:bold;">+':'>')+'$'+
     Math.abs(roundTo(s.maxrisk.g + results.initial.g,2,true))+"</span> at a price of $"+
     roundTo(s.maxrisk.p,2,true)+maxRiskDays)+(s.maxrisk.g + results.initial.g >= 0? '<br/><span class="minor">(calculator found no risk.  This can be due to low or high implied volatility, or out of date prices)</span>':'');

  mP = s.maxprofit.g < -results.initial.g;
  var maxProfitString = (s.maxprofit.g == 'u' ? 'infinite on upside' :
    '<span'+(mP ? ' style="color:#F00;">-':'>')+'$'+
    Math.abs(roundTo(s.maxprofit.g + results.initial.g,2,true))+"</span> at a price of $"+
    roundTo(s.maxprofit.p,2,true)+maxProfitDays)+(s.maxprofit.g + results.initial.g <= 0 ? '<br/><span class="minor">(calculator found no positive profit.  This can be due to low or high implied volatility, or out of date prices)</span>':'');
  ...
  var outlayString ='<span style="'+(mP ? 'color:#900;':'font-weight:bold;')+'">$'+Math.abs(roundTo(results.initial.g,2,true))+"</span>"+(mP? ' (net debit)': ' (net credit)');
...
function ele_spread_listen(tCall, foreignEle, localEle){
  ...
  for(var i in localEle.op_ele_ids){
    net += $('#'+ localEle.op_ele_ids[i]+'-price').val() * ($('#'+ localEle.op_ele_ids[i]+'-act').val() == 'sell' ? 1: -1);
    if (num === false) num = $('#'+ localEle.op_ele_ids[i]+'-num').val();
    else if (num !== $('#'+ localEle.op_ele_ids[i]+'-num').val()) num = -1;
  }

  $('#'+localEle.ele_id+'-spread').html(roundTo(net, 2, true) + " (net " + (net >= 0 ? "credit":"debit") + ")");
