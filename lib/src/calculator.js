...
function doCalculation(tNo, sType, cfg){
  ...
  if (errors){
    ...
    if ($('#t'+tNo+'_graph-newTab').attr('checked')){
      ...
      if (cfg['replaceWithLoader'] || !graphHadContent) {
      ...
      $('.popup').dialog('close');
      ajaxRequest("calcuating results", 'calculate', values, doCalculation_return, {tab_num:tNo, noScroll: !cfg['replaceWithLoader']});
    }
  }
}
...
function ivChangeChkUpdateNeeded(t_no) {
  ...
  var curVal = $('#t'+t_no+'_ivChange').val();
  if (parseFloat(newVal) !== curVal) {
...
function renderGraph(graphData, t_no){
  ...
  "   <span class='inputLabelSmall' style='position: relative; z-index: 10;'>IV change</span>"+
  "   <div style='position: relative;' onclick='ivChangeOnFocus("+t_no+")'>"+
  ...
  "   </div>"+
  // "   <span class='linkSubtle' id='t"+t_no+"_ivChange--val' style='position: relative; align-self: flex-end' onclick='ivChangeOnFocus("+t_no+")'>"+
  // "     "+(ivChangeCurVal === 0 ? '±' : ivChangeCurVal > 0 ? '+' : '')+ivChangeCurVal+"%"+
  // "   </span>"+
  ...
  "   >"+ 
  "   <input id='t"+t_no+"_ivChange' type='hidden' />"+
  '   <span>'+
  '    <input class="input inputNumeric noLeftMargin" '+
  "     style='width: 40px;'"+
  "     name='ivChange' id='t"+t_no+"_ivChange--input' "+
  "     onkeyup='ivChangeOnKeyup("+t_no+", event.target.value)'"+
  "    />%"+
  "   </span>"+
  ...
  if (graphData['graph_type'].substr(0, 9) == 'riskGraph'){
    output += '<div class="google_chart_holder" style="height:500px;"></div>\n\
      <script>\n\
 ...
 /* <p>Figures are "+
  ...
  output += "<div style='display: flex; margin-top: 5px'>"+
    ...
    '   <div>'+
    '     $<input data-app-inputset="priceRange" placeholder="auto" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" type="text" '+
    "       id=\""+t_no+"_graph-priceMin\" name=\"graph-priceMin\" value=\""+roundTo(graphData.graph_priceMin, 2)+"\" "+
    ...
    "       class=\"inputWithDollar inputNumeric input\" style=\"width:60px\""+
    "       onblur=\"graphRangeChange('min', "+t_no+", event);\" />" +
    '     &hyphen;<input data-app-inputset="priceRange" placeholder="auto" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" type="text" '+
    "       id=\""+t_no+"_graph-priceMax\" name=\"graph-priceMax\" value=\""+roundTo(graphData.graph_priceMax, 2)+"\" "+
    "       onchange=\"triggerListener('graphPriceRange', {ele_id:'t"+t_no+"_graph', tab_num:'"+t_no+"', ele_name:'graph'});\" "+
    "       onkeyup=\"triggerListener('graphPriceRange', {ele_id:'t"+t_no+"_graph', tab_num:'"+t_no+"', ele_name:'graph'});\" "+
    "       class=\"inputWithDollar inputNumeric input\" style=\"width:60px\""+
    ...
    "     <a href='javascript:void' onClick='updatePriceRange("+t_no+")') style='padding: 0.2em 0; text-decoration: none'><span style='text-decoration: underline'>Update price range</span>"+
...
function getSummary(results){
  ...
  var maxRiskString = (s.maxrisk.g === 'u' ? 'infinite <span class="minor">on upside</span>' :
  (s.maxrisk.p === 'i' ? formatPrice(Math.abs(s.maxrisk.g + results.initial.g)) + ' <span class="minor">on upside</span>' : 
  '<span'+(mP ? ' style="font-weight:bold;">+':'>')+
  formatPrice(Math.abs(s.maxrisk.g + results.initial.g))+"</span> <span class='minor'> at "+(results.underlying_symb)+
  formatPrice(s.maxrisk.p)+maxRiskDays + "</span>") +(s.maxrisk.g + results.initial.g >= 0? '<br/><span class="minor">(calculator found no risk.  This can be due to low or high implied volatility, or out of date prices)</span>':''));
  ...
  var timeTilExp = (results.data[Object.keys(results.data)[0]].length - 1) / 365;
  ...
  var maxProfitString = (s.maxprofit.g === 'u' ? 'infinite <span class="minor">on upside</span>' :
    (s.maxprofit.p === 'i' ? formatPrice(Math.abs(s.maxprofit.g + results.initial.g)) + ' <span class="minor">on upside</span>' : 
    '<span'+(mP ? ' style="color:#F00;">-':'>')+
    formatPrice(Math.abs(s.maxprofit.g + results.initial.g))+"</span>" + " <span class='minor'> at "+(results.underlying_symb)+
    formatPrice(s.maxprofit.p)+maxProfitDays + "</span>") +(s.maxprofit.g + results.initial.g <= 0 ? '<br/><span class="minor">(calculator found no positive profit.  This can be due to low or high implied volatility, or out of date prices)</span>':''));
  ...
  var outlayString ='<span style="'+(mP ? 'color:#900;':'font-weight:bold;')+'">'+formatPrice(Math.abs(results.initial.g))+"</span><span class='minor'>"+(mP? ' net debit': ' net credit') + "</span>";

  var summary =
    // "<span class='asAt minor'> (as at "+date("jS M Y", results.start_date)+")</span>"+
...
function initResults(tNo){
  ...
  var dialogOptions = {
    ...
    // dialogClass: viewStyle == 'mobile' ? null : 'dialog-fixed',
    dialogClass: "detailPopup",
    title:'Detail',
    maxHeight: 300,
    height: (180 > wh ? wh : 180),
    width: (450 > ww ? ww : 450),
    draggable:true,
  ...
  var ww = $(window).width(),
      ...
      dialogOptions = {
        ...
        open: function(){
          ...
          $('.ui-widget-overlay').fadeTo(0, 0.2).hide().fadeIn();
        },
        create: function (event) {
          $(event.target).parent().css('position', 'fixed');
        },
        position: { at: 'right-25 center' },
